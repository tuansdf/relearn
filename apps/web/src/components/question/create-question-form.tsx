...
import { CreateQuestionDto, IError, IQuestion } from "../../interface/types";

interface Props {
  onSuccess: () => void;
}

export default function CreateQuestionForm({ onSuccess }: Props) {
  const textId = useId();
  ...
  const questionMutation = useMutation<IQuestion, IError, CreateQuestionDto>(
    (data) => questionService.createInLesson(lessonId, data),
    {
      onSuccess: () => {
