...
import { ICourse, IError, UpdateCourseDto } from "../../interface/types";
...
export default function UpdateCourseForm({ course, onSuccess }: Props) {
  const titleId = useId();
  const descriptionId = useId();

  // form
  const { register, handleSubmit } = useForm<UpdateCourseDto>({
    defaultValues: {
      title: course.title,
  ...
  const courseMutation = useMutation<ICourse, IError, UpdateCourseDto>(
    (data) => courseService.update(course._id, data),
    {
      onSuccess: () => {
  ...
  const onSubmit = (data: UpdateCourseDto) => {
